...
function elt = mTkibam (task,bCap,temp)
    ...
    if(temp >= -5 && temp < 10)
        v1 = [-0.000000322047867                   0   0.000965281092581   0.998001288191470];
        CF = @(T) v1(1)*(T+5).^3 + v1(2)*(T+5).^2 + v1(3)*(T+5).^1 + v1(4);
    end
    if(temp >= 10 && temp < 25)
        v2 = [0.000001279695279  -0.000014492154037   0.000747898782027   1.011393593027415];
        CF = @(T) v2(1)*(T-10).^3 + v2(2)*(T-10).^2 + v2(3)*(T-10).^1 + v2(4);
    end
    if(temp >= 25 && temp < 32.5)
        v3 = [-0.000016298221080   0.000043094133509   0.001176928474103   1.023670311665434];
        CF = @(T) v3(1)*(T-25).^3 + v3(2)*(T-25).^2 + v3(3)*(T-25).^1 + v3(4);
    end
    if(temp >= 32.5 && temp <= 40)
        v4 = [0.000014382926257  -0.000323615840786  -0.000926984330475   1.028045508213049];
